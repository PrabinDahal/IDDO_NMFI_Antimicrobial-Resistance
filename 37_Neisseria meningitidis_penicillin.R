...
table(dat$antibiotic)

#-----------------------------------
# Look at pathogen-drug combination
#-----------------------------------
drugbug <- dat[which(dat$pathogen_2=="Neisseria meningitidis" & dat$antibiotic %in% c("Penicillin")),]
...
# Time period sub-group (1980s) from 1 study site had no events

# calculate wilson's 95% CI for the group with zero event
require(binom)
binom.confint(c(0),c(100), method="wilson")

# estimate the pooled proportion for rest of the time-period separately
