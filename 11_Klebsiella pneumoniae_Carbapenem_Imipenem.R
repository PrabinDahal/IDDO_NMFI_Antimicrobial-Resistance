...
#-----------------------------------
drugbug <- dat[which(dat$pathogen_2=="Klebsiella pneumoniae" & dat$antibiotic %in% c("Imipinem")),]
...
# Warning of "Cannot fit ML model" as there were no events in studies after 2000s 
# and only a single site before 2000s
# sub-group analysis was not carried out. 
# Instead a simple Wilson's 95% CI are presented

require(binom)
binom.confint(c(3,0,0), c(418,718,42), method="wilson")
...
# Time period sub-group (1990-1999) from 1 site with no events

binom.confint(c(0), c(8), method="wilson")
...
# When stratified by time-period, there is warning regarding convergence
# Model failed to converge with max|grad| = 0.0321081 (tol = 0.002, component 1)
