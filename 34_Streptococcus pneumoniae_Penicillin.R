...
#-----------------------------------
drugbug <- dat[which(dat$pathogen_2=="Streptococcus pneumoniae" & dat$antibiotic %in% c("Penicillin")),]
...
# Revised estimate after removing 1 observation from 1980-1989
...
require(binom)
binom.confint(0,41, method="wilson")
...
(asia1	<- metaprop(
			...
			total_invasive_isolates,
			studlab = site_code,
			control=list(stepadj=0.1, maxiter=1000,verbose=2)
			)
		)
update.meta(asia1, byvar=year_cat, comb.random = TRUE, comb.fixed = F)

# Warning of failure of model convergence 
# Increasing iteration number and stepadj doesnt solve the problem

# Time period sub-group (1990-1999) from two sites with no events
...
require(binom)
binom.confint(0,15, method="wilson")
